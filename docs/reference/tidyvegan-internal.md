...
tidyveganCovEllipse(cov, center = c(0, 0), scale = 1, npoints = 100)
tidyveganMahatrans(x, s2, tol = sqrt(.Machine$double.eps), na.rm = FALSE)
...
`tidyveganCovEllipse` finds the coordinates for drawing a covariance
ellipse.

`tidyveganMahatrans` transforms data matrix so that its Euclidean
distances are Mahalanobis distances. The input data `x` must be a matrix
centred by columns, and `s2` its covariance matrix. If `s2` is not
given, covariance matrix is found from `x` within the function. If
