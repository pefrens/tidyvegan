...
- what:
  ...
  [`rda`](https://pefrens.github.io/tidyvegan/reference/cca.md) know in
...
- arrows:
  ...
  ([`cca`](https://pefrens.github.io/tidyvegan/reference/cca.md) etc.).
...
it uses their `plot` method with option `type = "points"`. In addition,
the `plot` functions of these methods return invisibly an `ordiplot`
object which can be used by `identify.ordiplot` to label points. For
other ordinations it relies on
