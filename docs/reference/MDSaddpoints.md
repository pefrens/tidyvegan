...
Rectangular partition returns sampling units in `pick` against those not
in `pick` which is identical to distances between observations in two
data sets. Symmetric partitions are dissimilarities among observations
in , and are subsets of dissmilarities. The rectangular partitions can
be used to specify new observations in `MDSaddpoints`.
...
- pick:
  ...
  Indices (integers) of selected observations. The output be in the
  order of the input and will not be reordered by this argument.

- type:

  `"xy"` returns rectangular data of not picked against picked
  observations, and `"xx"` a subset of symmetric dissimilarities.

- invert:

  Invert `pick`, or drop elements listed.
...
- `cause`: Cause of termination of iterations. Integer for convergence
  criteria in
  [`monoMDS`](https://pefrens.github.io/tidyvegan/reference/monoMDS.md).
...
sampling units (rows) in two matries. In addition, analogue has distance
