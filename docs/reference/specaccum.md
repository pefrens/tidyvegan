...
- tidy:
  ...
  Instead of returning a `specaccum` object, return a `data.frame` of
  the results. This is useful for plotting with ggplot2.
...
[`rarefy`](https://pefrens.github.io/tidyvegan/reference/rarefy.md) with
...
If `tidy = FALSE` (default), function `specaccum` returns an object of
class `"specaccum"`, and `fitspecaccum` a model of class
`"fitspecaccum"` that adds a few items to the `"specaccum"` (see the end
of the list below).

If `tidy = TRUE`, `specaccum` returns a `data.frame` with columns
`Sites`, `Richness`, and (except for `method = "collector"`) `SD`. This
is useful for plotting with ggplot2.

The `specaccum` object has the following items:
...
[`rarefy`](https://pefrens.github.io/tidyvegan/reference/rarefy.md) and
[`rrarefy`](https://pefrens.github.io/tidyvegan/reference/rarefy.md) are
