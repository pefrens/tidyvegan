    ...
    function (X, Y = NULL, Z = NULL, scale = FALSE, ...)
{
    ...
    X <- .tidy_to_matrix(X, rownames.force = TRUE)
    if (!is.null(Y)) {
    ...
    ## printed if tidyvegan::rda object is displayed with klaR:::print.rda
    sol$regularization <- "this is a tidyvegan::rda result object"
    class(sol) <- c("rda", "cca")
    sol
}
