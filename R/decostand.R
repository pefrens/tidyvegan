    ...
    function (x, method, MARGIN, range.global, logbase = 2, na.rm = FALSE, ...)
{
    ...
    if (wasDataFrame) {
        ...
        if (any(non_numeric)) {
            num_cols <- which(!non_numeric)
            if (length(num_cols) == 0) {
                stop("No numeric columns found in the data frame")
            }
            x_num <- x[, num_cols, drop = FALSE]
            x_transformed <- decostand(x_num, method = method, MARGIN = MARGIN,
                                       range.global = range.global, logbase = logbase,
                                       na.rm = na.rm, ...)
            x_out <- x
            for (i in seq_along(num_cols)) {
                x_out[[num_cols[i]]] <- x_transformed[[i]]
            }
            attr(x_out, "parameters") <- attr(x_transformed, "parameters")
            attr(x_out, "decostand") <- attr(x_transformed, "decostand")
            return(x_out)
        }
    }
    x <- .tidy_to_matrix(x)
