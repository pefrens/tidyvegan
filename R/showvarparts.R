    ...
    function(parts, labels, bg = NULL, alpha=0.25, Xnames, id.size=1.2, ...)
{
    ...
    if (parts < 4) {
        ...
        ## Draw ellipses with tidyveganCovEllipse. Supply 2x2
        ...
        e2 <- tidyveganCovEllipse(matrix(c(d,-a,-a,d), 2, 2))
        e3 <- tidyveganCovEllipse(matrix(c(d, a, a,d), 2, 2))
        ...
        a <- (L-W)/2
        cnt <- sqrt(W/2)
        e1 <- tidyveganCovEllipse(matrix(c(d,-a,-a,d), 2, 2), c(-cnt, -cnt))
        e4 <- tidyveganCovEllipse(matrix(c(d, a, a,d), 2, 2), c( cnt, -cnt))
