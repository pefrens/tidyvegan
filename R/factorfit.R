    ...
    function (X, P, permutations = 0, strata = NULL, w,  ...)
{
    ...
    r <- NULL
    pval <- NULL
    totvar <- .Call(do_goffactor, X, rep(1L, NR), 1L, w, PACKAGE = "tidyvegan")
    ...
    for (i in seq_along(P)) {
        A <- as.integer(P[[i]])
        NL <- nlevels(P[[i]])
        invar <- .Call(do_goffactor, X, A, NL, w, PACKAGE = "tidyvegan")
        ...
        if (permutations) {
            ...
            ptest <- function(indx, ...) {
                take <- A[indx]
                invar <- .Call(do_goffactor, X, take, NL, w, PACKAGE = "tidyvegan")
                1 - invar/totvar
            }
