...
Several **vegan** functions can perform parallel processing using
the standard R package **parallel**.
The **parallel** package in R implements
the functionality of earlier contributed packages **multicore** and
**snow**.  The **multicore** functionality forks the analysis to
multiple cores, and **snow** functionality sets up a socket cluster
of workers.  The **multicore** functionality only works in unix-like
systems (such as MacOS and Linux), but **snow** functionality works
in all operating systems.  **Vegan** can use either method, but
defaults to **multicore** functionality when this is available,
...
`parallel`.  The normal default is `parallel = 1` which
means that no parallel processing is performed.  It is possible to set
parallel processing as the default in **vegan** (see
§\,\ref{sec:parallel:default}).

For parallel processing, the `parallel` argument can be either

* An integer in which case the given number of parallel processes
  will be launched (value $1$ launches non-parallel processing). In
  unix-like systems (*e.g.*, MacOS, Linux) these will be forked
  `multicore` processes. In Windows socket clusters will be set up,
  initialized and closed.
* A previously created socket cluster. This saves time as the
  ...
  up a socket cluster is discussed in §\,\ref{sec:parallel:socket}.

### Using parallel processing as default
\label{sec:parallel:default}

If the user sets option `mc.cores`, its value will be used as the
default value of the `parallel` argument in **vegan**
functions.  The following command will set up parallel processing to
all subsequent **vegan** commands:
```{r eval=FALSE}
options(mc.cores = 2)
```

The `mc.cores` option is defined in the **parallel** package,
but it is usually unset in which case **vegan** will default to
non-parallel computation.  The `mc.cores` option can be set by
the environmental variable `MC_CORES` when the **parallel**
package is loaded.

R allows setting up a default socket cluster
(`setDefaultCluster`), but this will not be used in **vegan**.

### Setting up socket clusters
...
the `parallel` argument in **vegan**.  If you want to use
socket clusters in unix-like systems (MacOS, Linux), this can be only
done with pre-defined clusters.

If socket cluster is not set up in Windows, **vegan** will create and
...
Most parallelized **vegan** functions work similarly in socket and
fork clusters, but in `oecosimu` the parallel processing is used
...
in parallel the `meandist` function of the `oecosimu`
...
library(parallel)
clus <- makeCluster(4)
clusterEvalQ(clus, library(tidyvegan))
mbc1 <- oecosimu(dune, meandist, "r2dtable",
                 parallel = clus)
stopCluster(clus)
```
Socket clusters are used for parallel processing in Windows, but you
do not need to pre-define the socket cluster in `oecosimu` if you
only need **vegan** commands.  However, if you need some other
contributed packages, you must pre-define the socket cluster also in
Windows with appropriate `clusterEvalQ` calls.

If you pre-set the cluster, you can also use **snow** style socket
clusters in unix-like systems.

### Random number generation

**Vegan** does not use parallel processing in random number
...
`library(tidyvegan)` with `clusterEvalQ` can take two seconds or
...
description of the user interface above (§\,\ref{sec:parallel:ui}).
Function `oecosimu` can be used as a reference implementation,
...
The value of the `parallel` argument can be `NULL`, a
positive integer or a socket cluster.  Integer $1$ means that no
parallel processing is performed.  The ``normal'' default is
`NULL` which in  the ``normal'' case is interpreted as $1$.  Here
``normal'' means that R is run with default settings without
setting `mc.cores` or environmental variable `MC_CORES`.

Function `oecosimu` interprets the `parallel` arguments in
the following way:

* `NULL`: The function is called with argument `parallel
    = getOption("mc.cores")`. The option `mc.cores` is normally
  unset and then the default is `parallel = NULL`.
* Integer: An integer value is taken as the number of created
  ...
  created, and if needed `library(tidyvegan)` is evaluated in the
...
* Socket cluster: If a socket cluster is given, it will be used in
...
(highest to lowest):
  ...
  * Explicitly given argument value of `parallel` will always
    be used.
  * If `mc.cores` is set, it will be used. In Windows this
    means creating and stopping socket clusters. Please note
    that the `mc.cores` is only set from the environmental
    variable `MC_CORES` when you load the **parallel** package,
    and it is always unset before first
    `require(parallel)`.
 * The fall back behaviour is no parallel processing.

# Nestedness and Null models
...
```
\caption{Matrix temperature for *Falco subbuteo* on Sibbo
...
the implementation in **vegan**.  However, there are still some open
issues, and probably **vegan** function `nestedtemp` will never
exactly reproduce results from other programs, although it is based on
the same general principles.\footnote{function `nestedness` in
  the **bipartite** package is a direct port of the original
  BINMATNEST program of \citet{RodGir06}.}  I try to give
main computation details in this document --- all details can be seen
in the source code of `nestedtemp`.

* Species and sites are put into unit square [RodGir06]. The
  row and column coordinates will be $(k-0.5)/n$ for $k=1 ... n$,
  ...
  * Species and sites are ordered alternately using indices
    [RodGir06]:
    \begin{equation}
    \begin{split}
...
t_j &= \sum_{i|x_{ij} = 0} (n-i+1)^2
    ...
    sorting should be ``a compromise'' [RodGir06] between these
    scores, and **vegan** uses $s+t$.  The result should be cool,
    but the packing does not try to minimize the temperature
    [RodGir06].  I do not know how the ``compromise'' is
  ...
  * The following function is used to define the fill line:
    ...
    numerically using R functions `integrate` and
    `uniroot`.  The fill line used in the original matrix
    temperature software [AtmarPat93] is supposed to be similar
    [RodGir06]. Small details in the fill line combined with
    differences in scores used in the unit square (especially in the
    corners) can cause large differences in the results.
  * A line with slope\,$= -1$ is drawn through the point and the $x$
    coordinate of the intersection of this line and the fill line is
    found using function `uniroot`. The difference of this
    intersection and the row coordinate gives the argument $d$ of matrix
    temperature (Fig. \ref{fig:nestedtemp}).
  * In other software, ``duplicated'' species occurring on every
    site are removed, as well as empty sites and species after
    reordering RodGir06. This is not done in **vegan**.

# Scaling in redundancy analysis
...
$\lambda_k$ are eigenvalues. In **vegan** the eigenvalues sum up to
...
\begin{table*}[t]
  \centering
  \caption{\label{tab:scales} Alternative scalings for rda used
    in the functions `prcomp` and `princomp`, and the
    one used in the **vegan** function `rda`
    and the proprietary software Canoco
    scores in terms of orthonormal species ($v_{ik}$) and site scores
    ($u_{jk}$), eigenvalues ($\lambda_k$), number of sites  ($n$) and
    species standard deviations ($s_j$). In `rda`,
    $\mathrm{const} = \sqrt[4]{(n-1) \sum \lambda_k}$.  Corresponding
    negative scaling in **vegan**
...
`stats::biplot, pc.biplot=TRUE` &
$u_{ik} \sqrt{n-1}$ &
$v_{jk} \sqrt{\lambda_k}$
`rda, scaling="sites"` &
$u_{ik} \sqrt{\lambda_k/ \sum \lambda_k} × \mathrm{const}$ &
$v_{jk} × \mathrm{const}$

`rda, scaling="species"` &
$u_{ik} × \mathrm{const}$ &
$v_{jk} \sqrt{\lambda_k/ \sum \lambda_k} × \mathrm{const}$  
`rda, scaling="symmetric"` &
$u_{ik} \sqrt[4]{\lambda_k/ \sum \lambda_k} × \mathrm{const}$ &
$v_{jk} \sqrt[4]{\lambda_k/ \sum \lambda_k} × \mathrm{const}$ 
`rda, correlation=TRUE` &
$u_{ik}^*$ &
...
the same graph.  If this graph is a graphical display of pca,
...
constant.  In functions `princomp`, `prcomp` and `rda`
with `scaling = "sites"`, the plotted scores define a biplot so that
...
invariant against scaling of data.  The solution in R
standard function `biplot` is to scale site and species scores
independently, and typically very differently
(Table~\ref{tab:scales}), but plot each independently to fill the
graph area.  The solution in Canoco and `rda` is to
...
The **vegan** package uses a scaling constant $c = \sqrt[4]{(n-1)
  \sum \lambda_k}$ in order to be able to use scaling by proportional
eigenvalues (like in Canoco) and still be able to have a
biplot scaling. Because of this, the scaling of `rda` scores is
non-standard. However, the `scores` function lets you to set
...
\begin{table*}[t]
  \centering
  \caption{\label{tab:rdaconst} Values of the `const` argument in
    **vegan** to get the scores that are equal to those from
    ...
    `tot.chi` in the `rda` result)}.
 \begin{tabular}{lccc}
  
\toprule
& **Scaling** &**Species constant** & **Site constant** 
\midrule
**vegan** & any  & $\sqrt[4]{(n-1) \sum \lambda_k}$ & $\sqrt[4]{(n-1) \sum \lambda_k}$
`prcomp`, `princomp` & `1` & $1$ & $\sqrt{(n-1) \sum_k \lambda_k}$
Canoco\,v3 & `-1, -2, -3` & $\sqrt{n-1}$ & $\sqrt{n}$
Canoco\,v4 & `-1, -2, -3` & $\sqrt{m}$ & $\sqrt{n}$
\bottomrule
\end{tabular}
\end{table*}

The scaling is controlled by three arguments in the `scores`
function in **vegan**:

  * `scaling` with options `"sites"`, `"species"`
    and `"symmetric"` defines the set of scores which is scaled
    by eigenvalues (Table~\ref{tab:scales}).
  * `const` can be used to set the numeric scaling constant
    to non-default values (Table~\ref{tab:rdaconst}).
  * `correlation` can be used to modify species scores so
...
# Weighted average and linear combination scores
...
\citet{Palmer93}.  However, functions `cca` and `rda` in
the **vegan** package use primarily WA scores. This chapter
explains the reasons for this choice.

Briefly, the main reasons are that

* LC scores *are* linear combinations, so they give us only
...
* \citet{McCune97} has demonstrated that noisy environmental
...
**vegan** have a choice of either LC or WA (default) scores, but
...
## LC Scores are Linear Combinations
...
```
Function `cca` in **vegan** uses WA scores as
...
the ordering of sites in species data?  Function `sample()` below
...
variables) and *independent of species data*: You can
...
*weighted* linear combinations with site totals of species data
...
max(residuals(proc))
```
In `cca` the difference would be somewhat larger than now
...
following example uses the classical dune meadow data Jongman87:
```{r}
data(dune)
data(dune.env)
...
*are* the environmental variables, and they can be distinct only
...
show where the site *should* be, the WA scores shows where the
site *is*.

Function `ordispider` adds line segments to connect each WA
...
analysis, too.  Moisture classes `1` and `2` seem to be
...
possible. Therefore **vegan** calls LC scores as
`constraints` and WA scores as `site scores`, and uses
