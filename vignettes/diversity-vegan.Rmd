...
\date{\footnotesize{
  processed with vegan \Sexpr{packageDescription("tidyvegan", field="Version")}
...
> This document explains diversity related methods in
  **vegan**. The methods are briefly described, and the equations
 ...
 The **vegan** package has two major components:
...
**Vegan** does not have indices for evenness (equitability), but
...
**vegan** also can estimate series of R\'{e}nyi and Tsallis
diversities. R{\'e}nyi diversity of order $a$ is [Hill73number]:
\begin{equation}
...
Tsallis diversity of order $q$ is [Tothmeresz95]:
\begin{equation}
...
We select a random subset of six sites for R\'{e}nyi diversities:
```{r}
k <- sample(nrow(BCI), 6)
R <- renyi(BCI[k,])
```
...
graphically using the standard `plot` function for the
`renyi` result (Fig.~\ref{fig:renyi}).
\begin{figure}
```{r fig.width=6, fig.height=6, echo=FALSE}
plot(R)
```
\caption{R\'{e}nyi diversities in six randomly selected plots.  Lines show
  the diversity profile for each plot.}
\label{fig:renyi}
\end{figure}

Finally, the $\alpha$ parameter of Fisher's log-series can be used as
a diversity index [FisherEtal43]:
```{r}
...
rarefied from $N$ to $n$ individuals is [Hurlbert71]:
\begin{equation}
\label{eq:rare}
...
$N$, and $q_i$ give the probabilities that species $i$ does *not* occur in a
sample of size $n$.  This is positive only when $N-x_i ≥ n$, but for
other cases $q_i = 0$ or the species is sure to occur in the sample.
The variance of rarefied richness is [HeckEtal75]:
\begin{multline}
\label{eq:rarevar}
s^2 = q_i (1-q_i)  
+ 2 \sum_{i=1}^S \sum_{j>i} \left[ \frac{{N- x_i - x_j
    \choose n}}{ {N
...
sample variant of Simpson's diversity index [Hurlbert71]\,--\,or
...
alternative to dubious diversity indices [Hurlbert71], but the
...
## Taxonomic diversity: average distance of traits

The two basic indices are called taxonomic diversity $\Delta$ and
taxonomic distinctness $\Delta^*$ [ClarkeWarwick98]:
\begin{align}
...
variation in taxonomic distinctness $\Lambda^+$ [ClarkeWarwick01]:
\begin{equation}
...
the reduction in the number of categories [ClarkeWarwick99]: if
...
Function `taxondive` implements indices of taxonomic diversity,
and `taxa2dist` can be used to convert classification tables to
...
data in **vegan**\footnote{Actually I made such a classification,
...
\caption{Taxonomic diversity $\Delta^+$ for the dune meadow data. The
  ...
  approximate confidence intervals ($2 ×$ standard error).}
\label{fig:taxondive}
\end{figure}
...
to all other species [PetcheyGaston06].

Function `treedive` implements functional diversity defined as
...
[PetcheyGaston02, PetcheyGaston06].  The example uses the
...
before `taxa2dist` converted them into distances)
```{r}
...
log-series [Preston48].  Instead of plotting species by
...
`prestonfit` and `prestondistr`.  Function `prestonfit`
...
4, 8, 16... times [WilliamsonGaston05]. Function
`prestonfit` can either split the ties or keep all limit cases in
the lower octave.  Function `prestondistr` directly maximizes
...
species [Whittaker65].  These are known as ranked abundance
distribution curves, species abundance curves, dominance--diversity
curves or Whittaker plots.  Function `radfit` fits some of the
most popular models [Bastow91] using maximum likelihood
estimation:
\begin{align}
...
abundances $a_r$, but there is no reason for this, and `radfit`
...
\caption{Ranked abundance distribution models for a random plot
  (no. \Sexpr{k}).  The best model has the lowest aic.}
\label{fig:rad}
\end{figure}

Function `radfit` compares the models using alternatively
...
parameters.  The penalty per parameter is $2$ in aic, and
$\log S$ in bic.  Brokenstick is regarded as a null model and
has no estimated parameters in **vegan**.  Preemption model has
...
Function `radfit` also works with data frames, and fits models
...
\label{eq:kindtvar}
s^2 = p_i (1 - p_i)  
+ 2 \sum_{i=1}^S \sum_{j>i} \left( r_{ij}
...
should be studied with respect to gradients [Whittaker60], but
almost everybody understand that as a measure of general heterogeneity
[Tuomisto10a, Tuomisto10b]: how many more species do you have in
...
richness per one site $\bar \alpha$ [Tuomisto10a]:
\begin{equation}
  \label{eq:beta}
  \beta = S/\bar \alpha - 1 \,.
\end{equation}
...
for all sites using **vegan** function `vegdist` with
binary data:
```{r}
...
`betadiver` [KoleffEtal03]. The indices in `betadiver`
...
Function `betadisper` can be used to analyse beta diversities
with respect to classes or factors [Anderson06, AndersonEtal06].
...
Functions `specpool` and `estimateR` implement some
methods of estimating the number of unseen species.  Function
`specpool` studies a collection of sites, and
`estimateR` works with counts of individuals, and can be used
...
occurrences $f_i = f_0, f_1, ..., f_N$, where $f$ is the number of
...
over-estimated [Good53]. The models for the pool size estimate
the number of species missing in the sample $f_0$.

Function `specpool` implements the following models to estimate
the number of missing species $f_0$. Chao estimator  is [Chao87, ChiuEtal14]:
\begin{equation}
\label{eq:chao}
\hat f_0 = \begin{cases}
...
$\frac{N}{N-1}$, but it was not originally used [Chao87].
...
\end{align}
The bootstrap estimator is [SmithVanBelle84]:
\begin{equation}
...
$\VAR(S_o) = 0$.  The variance of the Chao estimate is [ChiuEtal14]:
\begin{multline}
...
plots [SmithVanBelle84]:
\begin{equation}
\VAR(\hat f_0) = \left(\sum_{i=1}^N r_i^2 - \frac{f_1}{N}\right)
\frac{N-1}{N} \,.
\end{equation}
Variance of the second-order jackknife is not evaluated in
`specpool` (but contributions are welcome).

The variance of bootstrap estimator is[SmithVanBelle84]:
\begin{multline}
\VAR(\hat f_0) = \sum_{i=1}^{S_o} q_i (1-q_i)  
+2 \sum_{i \neq
...
## Pool size from a single site

The `specpool` function needs a collection of sites, but there
...
species with low frequencies.  Function `estimateR` implements
...
eq.~\ref{eq:chao} [Chao87, ChiuEtal14]:
\begin{equation}
  \label{eq:chao-bc}
...
sample size is the number of sites [ChiuEtal14].
...
\end{multline}
However, **vegan** does not use this, but instead the following more
...
\begin{multline}
  s^2 = \frac{1}{4} \frac{1}{(a_2+1)^4 S_p} [a_1 (S_p a_1^3
      a_2 + 4 S_p a_1^2 a_2^2 
+  2 S_p a_1 a_2^3 + 6 S_p a_1^2 a_2 + 2 S_p
      a_1 a_2^2 -2 S_p a_2^3 
+ 4 S_p a_1^2 + S_p a_1 a_2 -5 S_p a_2^2 - a_1^3 - 2
      a_1^2 a_2
- a_1 a_2^2 - 2 S_p a_1 - 4 S_p a_2 - S_p ) ]\,.
\end{multline}
The variance estimators only concern the number of unseen species like previously.

The ace is estimator is defined as [OHara05]:
\begin{equation}
\begin{split}
...
`veiledspec` estimates this integral from a model fitted either
with `prestondistr` or `prestonfit`, and fits the latter
...
Function `beals` implement Beals smoothing [McCune87,
  DeCaceresLegendre08]:
```{r}
smo <- beals(BCI)
```
...
plot(beals(BCI, species=j, include=FALSE), BCI[,j],
     ...
     xlab="Probability of occurrence")
```
\caption{Beals smoothing for *Ceiba pentandra*.}
\label{fig:beals}
\end{figure}
