...
* Package vignettes were converted from Sweave (PDF) to R Markdown (HTML), removing the dependency on Ghostscript during package checks.
...
* `plot.envfit` can use criterion `r2.min` for lower threshold of
  ...
  `plot.envfit` can optimize the location of text labels for factors
  and vectors (arrows).
...
* `MDSaddpoints`: Fixed an error in the examples where `dist2xy` was called with the incorrect argument `reverse = TRUE` instead of `invert = TRUE`.
