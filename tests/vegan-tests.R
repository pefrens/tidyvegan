...
###<--- BEGIN TESTS --->
suppressPackageStartupMessages(require(tidyvegan))
...
ev
### MDSaddpoints and dist2xy tests
data(dune)
d <- vegdist(dune)
## remove point 3 from ordination
mod3 <- metaMDS(dist2xy(d, 3, "xx", invert = TRUE), trace = 0)
## add point 3 to the result
add3 <- MDSaddpoints(mod3, dist2xy(d, 3))
stopifnot(inherits(add3, "nmds"))
stopifnot(nrow(add3$points) == 1)
stopifnot(rownames(add3$points) == "3")
## Use designdist2
d15 <- designdist(dune[1:15,])
m15 <- metaMDS(d15, trace = 0)
add15_20 <- MDSaddpoints(m15, designdist2(dune[1:15,], dune[16:20,]))
stopifnot(inherits(add15_20, "nmds"))
stopifnot(nrow(add15_20$points) == 5)
stopifnot(all(rownames(add15_20$points) == as.character(16:20)))
## Clean up
rm(d, mod3, add3, d15, m15, add15_20)
### end MDSaddpoints and dist2xy

### specaccum tidy tests
data(dune)
spa <- specaccum(dune, tidy = TRUE)
stopifnot(inherits(spa, "data.frame"))
stopifnot(all(c("Sites", "Richness", "SD") %in% names(spa)))
stopifnot(nrow(spa) == nrow(dune))
# collector method (no SD)
spa_coll <- specaccum(dune, method = "collector", tidy = TRUE)
stopifnot(inherits(spa_coll, "data.frame"))
stopifnot(all(c("Sites", "Richness") %in% names(spa_coll)))
stopifnot(!"SD" %in% names(spa_coll))
stopifnot(nrow(spa_coll) == nrow(dune))
# Clean up
rm(spa, spa_coll)
### end specaccum tidy tests
